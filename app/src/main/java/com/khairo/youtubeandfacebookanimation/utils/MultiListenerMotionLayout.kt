...
//        if (currentState == transitionId) return
            ...
            withTimeout(timeout) {
                suspendCancellableCoroutine<Unit> { continuation ->
                    val l = object : TransitionAdapter() {
                        override fun onTransitionCompleted(motionLayout: MotionLayout, currentId: Int) {
                            if (currentId == transitionId) {
                                removeTransitionListeners(this)
                    ...
                    continuation.invokeOnCancellation {
                        removeTransitionListeners(l)
                    }
                    // And finally add the listener
                    addTransitionListeners(l)
                    listener = l
                }
            }
    ...
    @Deprecated(message = "Use addTransitionListeners instead", replaceWith = ReplaceWith(
            "addTransitionListeners(listener)",
            "com.nikhilpanju.fabfilter.views.MultiListenerMotionLayout.addTransitionListeners"
    ))
    override fun setTransitionListener(listener: TransitionListener) {
        throw IllegalArgumentException("Use addTransitionListeners instead")
    }
}